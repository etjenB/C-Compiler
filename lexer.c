...
static void lex_finish_expression(){
    ...
    {
        compiler_error(lex_process->compiler, "An expression was closed that was never opened.\n");
    }
}
...
struct token* token_make_multiline_comment(){
    ...
    while (1)
    {
        ...
        if (c == EOF)
        {
            compiler_error(lex_process->compiler, "Multiline comment was opened but never closed.\n");
        }
        else if(c == '|'){
            nextc();
            if (peekc() == '/')
...
void lexer_validate_binary_string(const char* str){
    ...
    for (int i = 0; i < len; i++)
    {
        if (str[i] != '0' && str[i] != '1')
        {
            compiler_error(lex_process->compiler, "Invalid binary number encountered.\n");
        }
    }
}
...
struct token* token_make_quote(){
    ...
    if (nextc() != '\'')
    {
        compiler_error(lex_process->compiler, "Quotes were open but never closed.\n");
    }
